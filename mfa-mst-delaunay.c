...
void emst(Point *points[], int n, bool adjacency[]) {
    ...
    /* Delaunay */
    char *triswitches = "zNEeBQ"; // Q for quiet
    struct triangulateio *in = malloc(sizeof (struct triangulateio));
    double *listaPunti = malloc(sizeof (double) * 2 * n);
    for (int i=0; i<n; i++) {
        listaPunti[i*2] = points[i]->x;
        listaPunti[i*2+1] = points[i]->y;
    }
    in->pointlist = listaPunti;
    in->pointmarkerlist = NULL;
    in->numberofpoints = n;
    in->numberofpointattributes = 0;
    struct triangulateio *out = malloc(sizeof (struct triangulateio));
    out->edgelist = NULL; /* spazio allocato da Triangle */
    triangulate(triswitches, in, out, NULL);

    int numEdges = out->numberofedges;
    Edge **edges = malloc(sizeof (Edge*) * numEdges);
    for (int i=0; i<numEdges; i++) {
        int index1 = out->edgelist[i*2];
        int index2 = out->edgelist[i*2+1];
        edges[i] = newEdge(index1, index2, dist(points[index1], points[index2]));
    }    
    free(listaPunti);
    free(out->edgelist);
...
// locate already defined in triangle
Point *locateW(Point *terminals[], double weightsT[], Point *sp[], bool weightsS[], int n, double precision) {
...
int main() {
    ...
    while (satX < FINAL_SATURATION) {
        ...
        do {
            ...
            spostamentoMedio = 0;
            for (int s=0; s<n-2; s++) {
                newSp[s] = locateW(a, &x[s*n], sp, &y[s*(n-2)], 2*n-2, PRECISION_LOCATE_MID);
